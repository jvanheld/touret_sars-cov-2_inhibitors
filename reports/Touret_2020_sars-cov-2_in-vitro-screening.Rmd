...
inhibTable$pch <- 1 # point type for dot plots
inhibTable[inhibTable$wellType == "cellCtl", "pch"] <- 5 
inhibTable[inhibTable$wellType == "virusCtl", "pch"] <- 17
...
arbidolColor <- "black"
arbidolPch <- 19
inhibTable[arbidolWells, "color"] <- arbidolColor
inhibTable[arbidolWells, "pch"] <- arbidolPch
...
HOClSindex <- which(inhibTable$chemicalName == "Hydroxychloroquine sulfate")
HOClScolor <- "orange"
HOClSpch <- 13
inhibTable[HOClSindex, "color"] <- HOClScolor
...
```{r boxplots_per_plate_raw, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of raw viability values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates. Orange square: Hydroxychloroquine sulfate. "  }
#### Boxplots per plate  ####
...
stripchart(viability ~ Plate, vertical = FALSE, 
           ...
           col = arbidolColor, pch = arbidolPch)
...
```{r boxplots_per_plate_iqNormV, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of inter-quartile standardized viability (iqNormV) values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates. Orange square: Hydroxychloroquine sulfate. "}
...
```{r iqNormV_ranked, fig.width=7, fig.height=12, out.width="80%", fig.cap="Values of the plate-wise IQR-standardized viability index for all the tested molecules. Molecules are colored according to the plate number. A: virus control (infected untreated); B: treated cells; C: cell control (untreated cells); D: ranked values (all types)"}
...
```{r compa_viability_scores_dotplots, fig.width=12, fig.height=11, out.width="100%", fig.cap="Comparison between viability scores. "  }
...
I strongly recommend to use the plate-wise normalized inhibition index in order to select the candidate molecules. 

With an adjusted p-value of `r alpha`,  `r nrow(selected)` molecules are declared significant and could be considered as candidate for further characterization. 
