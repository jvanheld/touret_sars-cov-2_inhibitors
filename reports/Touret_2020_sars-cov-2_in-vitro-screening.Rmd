...
```{r boxplots_per_plate, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of raw viability values per plate"}
#### Boxplots per plate  ####
...
### Normalised viability plots



```{r normV_plot, fig.width=7, fig.height=12, out.width="80%", fig.cap="Values of the plate-wise relative viability index for all the tested molecules. Molecules are colored according to the plate number. A: virus control (infected untreated); B: treated cells; C: cell control (untreated cells); D: ranked values (all types)"}
...
- scaling: divide by an estimator of the plate-wise standard deviation
...
This relies on the assumption that, *in each plate*, the number of active molecule (statistical outliers). Since teach plate contains tests of 80 molecules, there are 19 molecules above the third quartile ($Q3$). However, we know that the plates were manufactured with some grouping of molecules of the same structural family.  It is not impossible that some plate contains more than 19 molecules having an inhibitory effect. Such a situation would result in a loss of sensitivity, since the presence of active molecules in the inter-quartile range would lead to over-estimate the dispersion. 
...
statPerPlate$scaling <- (statPerPlate$TrMedian - statPerPlate$TrQ1) / 
   (qnorm(p = 0.5) - qnorm(p = 0.25))

kable(statPerPlate, caption = "Plate-wise statistics of treated cells")
...
inhibTable$normV <- (inhibTable$viability - statPerPlate[plate, "TrMedian"]) / statPerPlate[plate, "scaling"]
# IQR(inhibTable$normV)
# IQR(rnorm(n = 1000000))
...
normVstat <- data.frame(
  mean = mean(inhibTable$normV),
  sd = sd(inhibTable$normV),
  IQR = IQR(inhibTable$normV),
  var = var(inhibTable$normV),
  min = min(inhibTable$normV),
  Q1 = as.vector(quantile(inhibTable$normV, probs = 0.25)),
  median = median(inhibTable$normV),
  Q3 = as.vector(quantile(inhibTable$normV, probs = 0.75)),
  max = max(inhibTable$normV)
)
...
```{r normV_distrib, fig.width=7, fig.height=5, out.width="60%", fig.cap="Distribution of the plate-wise normalised viability. Top: virus control (infected, untreated); middle: treated; bottom: cell control (untreated)"}
#### Histograms of normalised viability ####
histBreaks = seq(from = floor(min(inhibTable$normV)), 
                 to = ceiling(max(inhibTable$normV)), by = 0.1)

par(mfrow = c(3,1))
hist(inhibTable[wellType == "virusCtl", "normV"], 
     main = "Virus control – normalised viability",
     breaks = histBreaks, xlab = "Normalised viability", 
     col = "orange", border = "orange")
hist(inhibTable[wellType == "treated", "normV"], 
     main = "Treated cells – normalised viability",
     breaks = histBreaks, xlab = "Normalised viability", 
     col = "grey", border = "grey")
abline(v = mean(inhibTable[wellType == "treated", "normV"]), col = "blue")
abline(v = median(inhibTable[wellType == "treated", "normV"]), col = "darkgreen")
legend("topright", legend = c("mean", "median"),
       col = c("blue", "darkgreen"),
       lwd = 2)
hist(inhibTable[wellType == "cellCtl", "normV"], 
     main = "Cell control (untreated) – normalised viability",
     breaks = histBreaks, xlab = "Normalised viability", 
...
legend("topright", 
       ...
       col = plateColor, pch = 1, 
       cex = 0.7)
plot(inhibTable[wellType == "treated", "normV"],
     panel.first = grid(),
     main = "Normalised viability (normV)",
     xlab = "Molecules",
     ylab = "normV",
     col = inhibTable[wellType == "treated", "color"],
     cex = 0.3,
     xlim = c(0, length(inhibTable[wellType == "treated", "normV"])*1.05)
     )
legend("topright", 
       legend = names(plateColor), 
       col = plateColor, pch = 1, 
       cex = 0.7)
plot(inhibTable[wellType == "cellCtl", "normV"],
     panel.first = grid(),
     main = "Cell control (uninfected)",
     xlab = "Replicates, sorted per plate",
     ylab = "normV",
...
legend("topright", 
       ...
       cex = 0.7)
# names(supTable)
normVrank <- order(inhibTable$normV, decreasing = TRUE)
plot(inhibTable[normVrank, "normV"],
     main = "Ranked normalised viability values",
     xlab = "Molecules (ranked by normV index)",
     ylab = "normV",
     col = inhibTable[normVrank, "color"],
     cex = 0.3,
     panel.first = grid(),
     xlim = c(0, length(normVrank) * 1.05)
     )
legend("topright", 
       legend = names(plateColor), 
       col = plateColor, pch = 1, 
...
The plot of normalised viability values (top panel) clearly shows that the plate-wise normalisation suppressed the background bias. 
...
inhibTable$p.value <- pnorm(inhibTable$normV, mean = 0, sd = 1, lower.tail = FALSE)
...
```{r normVpval_histogram, fig.width=7, fig.height=5, out.width="60%", fig.cap="Histogram of the nominal (unadjusted) p-values derived from the plate-wise normalised inhibition index. "}
...
```{r normV_manhattan_plot, fig.width=7, fig.height=4, out.width="80%", fig.cap="Volcano plot. "}
#### Manhattan plot ####
...
```{r normV_volcano_plot, fig.width=7, fig.height=7, out.width="60%", fig.cap="Volcano plot. "}
#### Volcano plot ####
