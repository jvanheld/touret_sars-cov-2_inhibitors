...
The viability is measured as the proportion of living cells in a given condition (infection, treatment).
...
- $A$ is the number of cells alive
- $V$ is denoted hereafter as **raw viability**
...
```{r boxplots_per_plate_raw, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of raw viability values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates.  "  }
#### Boxplots per plate  ####
...
stripchart(viability ~ Plate, vertical = FALSE, 
...
We performed a log2 transformation of the raw viability measures in order to normalize them. 

```{r boxplots_per_plate_log2, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of log2-transformed viability values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates. "}
...
## Add points to denote the arbidol controls
stripchart(log2V ~ Plate, vertical = FALSE, 
           data = inhibTable[arbidolWells, ],
           method = "jitter", add = TRUE, cex = 0.7, 
           col = markColor["Arbidol"], pch = markPCh["Arbidol"])
...
```{r boxplots_per_plate_Vr, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of relative viability (Vr) values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates.  "}
...
## Add points to denote the arbidol controls
stripchart(Vr ~ Plate, vertical = FALSE, 
           data = inhibTable[arbidolWells, ],
           method = "jitter", add = TRUE, cex = 0.7, 
           col = markColor["Arbidol"], pch = markPCh["Arbidol"])
...
We perform a plate-wise normalization with a centering on the median and a scaling to compensate for the plate-wise variance effect denoted above. To this purpose, we compute z-scores from the original value. 
...
statPerPlate <- data.frame(
  ...
  arbidolMean = as.vector(by(
    data = inhibTable[arbidolWells, "viability"], 
...
$$z_{c,i} = \frac{v_c - \hat{\mu_i}}{\hat{\sigma_i}} = (v_c - \tilde{v_i}) \frac{Q3_N - Q1_N}{Q3_i - Q3_i} = (v_c - \tilde{v_i}) \cdot S_i$$
...
## standardized relative to the same difference in a normal distribution.
...
```{r iqNormV_distrib, fig.width=7, fig.height=5, out.width="60%", fig.cap="Distribution of the plate-wise IQR-standardized viability. Top: virus control (infected, untreated); middle: treated; bottom: cell control (untreated)"}
...
hist(inhibTable[wellType == "virusCtl", "iqNormV"], 
     ...
     breaks = histBreaks, xlab = "IQR-standardized viability", 
...
```{r boxplots_per_plate_iqNormV, fig.width=7, fig.height=10, out.width="80%", fig.cap="Distribution of inter-quartile standardized viability (iqNormV) values per plate. Top virus control (untreated infected cells); middle: treated cells; bottom: cell control (uninfected). Black plain circles: arbidol control duplicates.  "}
...
## Add points to denote the arbidol controls
stripchart(iqNormV ~ Plate, vertical = FALSE, 
           data = inhibTable[arbidolWells, ],
           method = "jitter", add = TRUE, cex = 0.7, 
           col = markColor["Arbidol"], pch = markPCh["Arbidol"])
...
- The cell control box plots show wide variation in their medians and dispersion: 
    ...
    - Even for the plates where the cell controls have a high viability after IQR-based standardization, there are strong between-plates variations. 

This standardization seems thus efficient to correct the apparent over-dispersion of plates 11 to 19, and thereby reduce the rate of likely false positives, but the wide between-plate variability of the untreated cells suggest that the resulting z-scores should not be interpreted as indicators of viability.
...
plot(inhibTable[wellType == "treated", "iqNormV"],
     panel.first = grid(),
     main = "IQR-standardized viability (iqNormV)",
     xlab = "Molecules",
     ylab = "iqNormV",
     ylim = yRange,
     col = inhibTable[wellType == "treated", "color"],
...
plot(inhibTable[iqNormVrank, "iqNormV"],
     ...
     pch = inhibTable[iqNormVrank, "pch"],
     cex = 0.5, las = 1,
     panel.first = grid(),
...
```{r iqNormVpval_histogram, fig.width=7, fig.height=5, out.width="60%", fig.cap="Histogram of the nominal (unadjusted) p-values derived from the plate-wise IQR-standardized viability index. "}
...
# sum(arbidolWells)
kable(inhibTable[arbidolWells, c("Plate", "viability", "Vr", "iqNormV", "log10FDR")], caption = "Viability indices for the arbidol controls (2 replicates per plate) ")
```
...
## Mark cell controls
points(inhibTable[wellType == "cellCtl", c("Vr", "iqNormV")], 
       col = markColor["cellCtl"], pch = markPCh["cellCtl"], cex = 0.5)
## Mark virus controls
points(inhibTable[wellType == "virusCtl", c("Vr", "iqNormV")], 
       col = markColor["virusCtl"], pch = markPCh["virusCtl"], cex = 0.5)
## Mark arbidol controls
points(inhibTable[arbidolWells, c("Vr", "iqNormV")], 
       col = markColor["Arbidol"], pch = markPCh["Arbidol"], cex = 0.5)
## Mark Hydroxychloroquine
points(inhibTable[HOClSindex, c("Vr", "iqNormV")], 
       col = markColor["Hydroxychloroquine"], pch = markPCh["Hydroxychloroquine"], cex = 0.5)
