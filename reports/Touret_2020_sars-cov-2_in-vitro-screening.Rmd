...
```{r z_ranked, fig.width=7, fig.height=12, out.width="80%", fig.cap="Values of the plate-wise IQR-standardized inhibition index ($z$) for all the tested molecules. Molecules are colored according to the plate number. A: virus control (infected untreated); B: treated cells; C: cell control (untreated cells); D: ranked values (all types). Plain triangles: virus control (untreated infected cells). Black plain circles: arbidol control duplicates per plate. Orange square: Hydroxychloroquine sulfate. "}
...
plot(x = -inhibTable[wellType == "treated", "log10FDR"], 
     ...
     cex = 0.5)
abline(h = 0)
abline(h = seq(0, -min(inhibTable[wellType == "treated", "log10FDR"]), 1), lty = "dotted", col = "grey")
abline(v = (0:19) * 82, col = "grey")
mtext(plateNumbers, at = (1:19) * 82 - 41, side = 1)
...
#### Volcano plot ####
plot(x = inhibTable[wellType == "treated", "I"],
...
# names(inhibTable)
kable(inhibTable[arbidolWells, c("Plate", "CTB", "Vratio", "I", "z", "log10FDR")], caption = "inhibition indices for the arbidol controls (2 replicates per plate) ")
```
...
```{r compa_dotplot_CTB_vs_I, fig.width=8, fig.height=8, out.width="100%", fig.cap="Comparison between viability scores. CTB versus Inhibition index ($I$). Diamonds: cell control (uninfected cells). Plain triangles: virus control (untreated infected cells). Black plain circles: arbidol control duplicates per plate. Orange square: Hydroxychloroquine sulfate. "  }
...
#### Dot plot: viability ratio versus inhibition index ####
plot(inhibTable[, c("CTB", "I")],
     main = "CTB vs inhibition index",
     xlab = "Cell Titer Blue intensity (CTB)", 
     ylab = "Inhibition index (I)", 
...
abline(v = seq(from = 0, to = 1.4, by = 0.1), col = "#DDDDDD")
abline(v = 1)
...
```{r compa_dotplot_I_vs_z, fig.width=8, fig.height=8, out.width="100%", fig.cap="Comparison between viability scores. Inhibition index versus IQR-standardized inhibition (z-score). Diamonds: cell control (uninfected cells). Plain triangles: virus control (untreated infected cells). Black plain circles: arbidol control duplicates per plate. Orange square: Hydroxychloroquine sulfate. "  }


#### Dot plot: IQR-standardized versus inhibition index ####
plot(inhibTable[, c("I", "z")],
     main = "Inhibition index vs IQR-standardized inhibition",
     xlab = "Inhibition index (I)",
     ylab = "IQR-standardized inhibition (z-score)",
     col = inhibTable[, "color"],
     pch = inhibTable[, "pch"],
     cex = 0.5,
     panel.first = grid(),
...
## Mark cell controls
points(inhibTable[wellType == "cellCtl", c("I", "z")], 
       col = markColor["cellCtl"], pch = markPCh["cellCtl"], cex = 0.5)
## Mark virus controls
points(inhibTable[wellType == "virusCtl", c("I", "z")], 
       col = markColor["virusCtl"], pch = markPCh["virusCtl"], cex = 0.5)
## Mark arbidol controls
points(inhibTable[arbidolWells, c("I", "z")], 
       col = markColor["Arbidol"], pch = markPCh["Arbidol"], cex = 0.5)
## Mark Hydroxychloroquine
points(inhibTable[HOClSindex, c("I", "z")], 
       col = markColor["Hydroxychloroquine"], pch = markPCh["Hydroxychloroquine"], cex = 0.5)
...
plot(x = inhibTable[, "Vr"],
     ...
     main = "inhibition index vs FDR from iqrNormV",
