...
author: "Franck Touret, Magali Gilles, Karine Barral, Antoine Nougaire1, Etienne Decroly, Xavier de Lamballerie, and Bruno Coutard (2020). Statistical analysis by Jacques van Helden"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
## Three-points normalization with arbidol

```{r arbidol_viability_indices}
#### Viability indices for the arbidol contron ####
# sum(arbidolWells)
kable(inhibTable[arbidolWells, c("Vr", "iqNormV", "log10Padj")], caption = "Viability indices for the arbidol controls (2 replicates per plate) ")
```
...
```{r compa_viability_scores_dotplots, fig.width=10, fig.height=12, out.width="100%", fig.cap="Comparison between viability scores. Diamonds: cell control (uninfected cells). Plain triangles: virus control (untreated infected cells). Black plain circles: arbidol control duplicates per plate. Orange square: Hydroxychloroquine sulfate. "  }
...
inhibTable$selected.FDR <- as.numeric(inhibTable$padj < alpha)
...
inhibTable$selected.onlyFDR <- inhibTable$selected.FDR & !inhibTable$selected.ii
inhibTable$selected.onlyii <- !inhibTable$selected.FDR & inhibTable$selected.ii
inhibTable$selected.both <- inhibTable$selected.FDR & inhibTable$selected.ii
inhibTable$selected.either <- inhibTable$selected.FDR | inhibTable$selected.ii

criteria <- c("FDR", "ii", "both", "either", "onlyFDR", "onlyii")

candidatesPerPlate <- data.frame(matrix(nrow = nbPlates, ncol = length(criteria), 0))
...
```{r pval_histogram, fig.width=7, fig.height=5, out.width="60%", fig.cap="Histogram of the nominal (unadjusted) p-values derived from the plate-wise normalized inhibition index. "}
...
- comptage des positifs par plaque avec les différentes méthodes + plots
